...
flowchart TB
    ...
    BOSS["🧭 강팀장<br/><i>(ceo-advisor)<br/>스파링·우선순위·회의록</i>"]

    PM["🎯 강팀장<br/><i>(pm)<br/>허브 · 태스크 분해 · 라우팅</i>"]

    subgraph EXEC["실행 라인 — 베이스: Crowny Class 디자인 시스템"]
        direction LR
        UX["🧪 강디<br/>(ux-designer)<br/>플로우·IA"]
        UI["🎨 강디<br/>(ui-designer)<br/>시각·컴포넌트"]
        DEV["⚙️ 강개발<br/>(developer)<br/>구현·디버깅"]
        QA["🔍 강체크<br/>(qa)<br/>검증·회귀"]
    end

    MKT["📣 아뱅<br/><i>(marketer)<br/>카피·채널·캠페인</i>"]
    SEC["🛡️ 강체크<br/><i>(security)<br/>리스크·우회안</i>"]
...
sequenceDiagram
    ...
    participant S as 🧭 강팀장
    participant P as 🎯 강팀장
    participant B as 💡 아뱅
    participant D1 as 🧪 강디
    participant D2 as 🎨 강디
    participant H as 📣 아뱅
    participant DV as ⚙️ 강개발
    participant SC as 🛡️ 강체크
