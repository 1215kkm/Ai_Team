...
# → 단계 2처럼 Secrets 등록, 단계 3처럼 트리거 설정
```

만들어지는 것:
- `.claude/` (강팀 전체 정의)
- `templates/` (회의 템플릿)
- `scripts/` (start-meeting, send-meeting, setup-telegram 등)
- `styles/tokens.css` (Crowny Class 디자인 시스템 변수 한 파일)
- `.ai-team/state.json` (상태 머신 초기값)
- `.github/workflows/telegram-poll.yml` (별도 복사 필요 — 아래)
- `CLAUDE.md` (이 프로젝트 전용 룰)

`telegram-poll.yml` 은 Ai_Team 레포에서 수동 복사:
```bash
mkdir -p .github/workflows
curl -fsSL https://raw.githubusercontent.com/1215kkm/Ai_Team/main/.github/workflows/telegram-poll.yml \
